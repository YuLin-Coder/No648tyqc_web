 ...
 <script>
  ...
  layui.use(['layer','form','table','jquery'], function () {//调用layui组件
	    ...
	    table.render({
            elem: '#demo'  //绑定table id
            	,url: '${pageContext.request.contextPath}/selectcatData' //数据接口
            , cellMinWidth: 80
            ,cols: [[ //表头
	           /*  {field: 'id', title: 'ID'	}
	    ...
	    table.on('tool(test)',function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
	 	...
	 	function del(id){
			 $.ajax({
				  type: 'POST',
				  url: '${pageContext.request.contextPath}/delcatInfo',
				  data: {"id":id},
				  /* dataType: "json", */
	 	...
	 	function edit(id){
	 		layer.open({
	 	          type: 2
	 	          ,title: '修改类型'
	 	          ,content: '${pageContext.request.contextPath}/toEditcatAct?id='+id
	 	          ,maxmin: true
	 	          ,area: ['500px', '450px']
	 	          ,btn: ['确定', '取消']
	 	          ,yes: function(index, layero){
	 	           // alert("1111");
	 	        	...
	 	        	$.ajax({
	 					  type: 'POST',
	 					  url: '${pageContext.request.contextPath}/editcatInfo',
  ...
  function add(){
		layer.open({
	          type: 2
	          ,title: '添加类型'
	          ,content: '${pageContext.request.contextPath}/toAddcatAct'
	          ,maxmin: true
	          ,area: ['500px', '450px']
	          ,btn: ['确定', '取消']
	          ,yes: function(index, layero){
	           // alert("1111");
	        	...
	        	$.ajax({
					  type: 'POST',
					  url: '${pageContext.request.contextPath}/addcatInfo',
					  ...
					  success:function(req){
					        ...
					        if(req=='success'){
					        	layer.alert("操作成功");
					        	window.location.href= '${pageContext.request.contextPath}/catPage';
					        }else if(req=='repeat'){
					        	layer.alert("学号重复");
					        }else{
					        	layer.alert("服务器错误，请重试");
